...
export default function Transactions() {
  ...
  const handleExportJSON = () => exportJSON(filtered);
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          {role === 'admin' ? (
            <>
              ...
              <button onClick={handleImportClick} className="flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                <Upload size={14} /> Import CSV
              </button>
              <button onClick={handleExportCSV} className="flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                <Download size={14} /> Export CSV ({filtered.length})
              </button>
              <button onClick={handleExportJSON} className="flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
      ...
      <AddEditModal isOpen={modalOpen} onClose={handleModalClose} transaction={editTx} />
    </div>
  );
}
